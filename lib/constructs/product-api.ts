...
export class ProductApi extends Construct {
  ...
  constructor(scope: Construct, id: string, props: ProductApiProps) {
    ...
    products.addMethod('POST', 
      new apigateway.LambdaIntegration(props.postItemFunction), 
      { apiKeyRequired: true }
    );
    
    // GET /products
    products.addMethod('GET', 
      new apigateway.LambdaIntegration(props.getItemFunction)
    );
    ...
    productItem.addMethod('PUT', 
      new apigateway.LambdaIntegration(props.putItemFunction), 
      { apiKeyRequired: true }
    );

    // GET /products/{category}/{productId}/translation
    const translation = productItem.addResource('translation');
    translation.addMethod('GET', 
      new apigateway.LambdaIntegration(props.translateItemFunction)
    );
  }
}